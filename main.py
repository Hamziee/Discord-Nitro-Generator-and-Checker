...
import time
LICNECE = """
Copyright (c) 2022 Hamzie#1000 hamzie@email.com
...
try:  # Check if the requrements have been installed
    ...
    input(
        f"Module discord_webhook not installed, to install run '{'python3' if os.name == 'nt' else 'python3.8'} -m pip install discord_webhook'\nYou can ignore this error if you aren't going to use a webhook.\nPress enter to continue.")
    USE_WEBHOOK = False
try:  # Setup try statement to catch the error
    ...
    # Tell the user it has not been installed and how to install it
    input(
        f"Module requests not installed, to install run '{'python3' if os.name == 'nt' else 'python3.8'} -m pip install requests'\nPress enter to exit")
...
try:  # Setup try statement to catch the error
    ...
    # Tell the user it has not been installed and how to install it
    input(
        f"Module numpy not installed, to install run '{'python3' if os.name == 'nt' else 'python3.8'} -m pip install numpy'\nPress enter to exit")
    exit()  # Exit the program
    ...
    def main(self):  # The main function contains the most important code
        ...
        if os.name == "nt":  # If the system is windows
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Nitro Generator and Checker - Made by Hamzie#1000")  # Change the
        else:  # Or if it is unix
            print(f'\33]0;Nitro Generator and Checker - Made by Hamzie#1000\a',
...
╚═╝░░╚═╝╚═╝░░╚═╝╚═╝░░░░░╚═╝╚══════╝╚═╝╚══════╝  ╚═════╝░░╚════╝░╚═╝░░░░░░░░╚═╝░░░░░░╚═╝░░░╚═╝░░╚═╝░░╚═╝╚═╝░░╚═╝╚══════╝
        ...
        self.slowType("Made by: Hamzie#1000", .02)
        ...
        for s in c:  # Loop over the amount of codes to check
            ...
            if os.name == "nt":  # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by Hamzie#1000")  # Change the title
                print("")
            else:  # If it is a unix system
                # Change the title
                print(
                    f'\33]0;Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by Hamzie#1000\a', end='', flush=True)
        ...
        if response.status_code == 200:  # If the responce went through
            ...
            if notify is not None:  # If a webhook has been added
                DiscordWebhook(  # Send the message to discord letting the user know there has been a valid nitro code
                    url=url,
                    content=f"Valid Nito Code detected! @everyone (Thank me :>) \n{nitro}"
                ).execute()
